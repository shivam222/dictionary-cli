...
const options =  ['1: try again','2: get a hint','3: quit'];
 ...
 const play = (res, expected_answer_set, covered_hints) => {
   return new Promise ((resolve, reject) => {
    logger.logErr("wrong answer:( try again");
    inquirer
    .prompt([
      {
        type: 'list',
...
return new Promise ((resolve, reject) => {
 ...
 if (cand.length !== 0) {
    ...
    if(choice === 'syn') {
     ...
     logger.logInfo(`Your hint, answer is jumbled form of the following word: ${jumbled}`);
 }
 return takeUserAnswer(res, expected_answer_set, covered_hints)
 .then( (result) => {
     resolve(result);
...
const giveAnswerAndExit = (res, expected_answer_set) => {
    return new Promise ((resolve) => {
        logger.logInfo(`expected answer list: ${expected_answer_set} \n definition: ${res.def[0].text}`);
        resolve(false);
    });
};
